function varargout = Land_Use_Rt_Transact_Mgmt_Sys(varargin)
% LAND_USE_RT_TRANSACT_MGMT_SYS MATLAB code for Land_Use_Rt_Transact_Mgmt_Sys.fig
%      LAND_USE_RT_TRANSACT_MGMT_SYS, by itself, creates a new LAND_USE_RT_TRANSACT_MGMT_SYS or raises the existing
%      singleton*.
%
%      H = LAND_USE_RT_TRANSACT_MGMT_SYS returns the handle to a new LAND_USE_RT_TRANSACT_MGMT_SYS or the handle to
%      the existing singleton*.
%
%      LAND_USE_RT_TRANSACT_MGMT_SYS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in LAND_USE_RT_TRANSACT_MGMT_SYS.M with the given input arguments.
%
%      LAND_USE_RT_TRANSACT_MGMT_SYS('Property','Value',...) creates a new LAND_USE_RT_TRANSACT_MGMT_SYS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Land_Use_Rt_Transact_Mgmt_Sys_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Land_Use_Rt_Transact_Mgmt_Sys_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Land_Use_Rt_Transact_Mgmt_Sys

% Last Modified by GUIDE v2.5 22-Feb-2024 14:02:11

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Land_Use_Rt_Transact_Mgmt_Sys_OpeningFcn, ...
                   'gui_OutputFcn',  @Land_Use_Rt_Transact_Mgmt_Sys_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Land_Use_Rt_Transact_Mgmt_Sys is made visible.
function Land_Use_Rt_Transact_Mgmt_Sys_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Land_Use_Rt_Transact_Mgmt_Sys (see VARARGIN)

% Choose default command line output for Land_Use_Rt_Transact_Mgmt_Sys
...
function varargout = Land_Use_Rt_Transact_Mgmt_Sys_OutputFcn(hObject, eventdata, handles) 
...
set(handles.text26, 'String', '  土地使用权交易管理系统');

handles.uipanel2.Title = '交易类型管理';
handles.uipanel3.Title = '交易管理设置';
handles.uipanel4.Title = '交易数据管理';
handles.uipanel5.Title = '交易流程设置';
handles.uipanel6.Title = '交易效果评估';

set(handles.pushbutton8, 'String', '确认选择');
set(handles.pushbutton9, 'String', '确认设置');
set(handles.pushbutton10, 'String', '保存信息');
set(handles.pushbutton11, 'String', '确认设置');
set(handles.pushbutton12, 'String', '效果评估');

set(handles.text_btn_text_1, 'String', '导入土地使用权交易流程图');
set(handles.text_btn_text_2, 'String', '绘制土地使用权交易效果分析三维图');
set(handles.text_btn_text_3, 'String', '绘制交易效果评估分析柱状图');
unit_content = {'', '', '', '', '分'};

text_contents1 = {'出售土地使用权', '出租土地使用权', '土地使用权转让', '土地抵押', '土地租赁', '土地互换', '拍卖土地使用权'};											
text_contents2 = {'交易记录管理', '交易流程跟踪', '合同管理', '支付管理', '交易风险评估', '法律法规遵守', '用户验证'};											
text_contents3 = {'交易案例总数', '平均交易金额', '交易成功率', '最活跃交易区域', '交易争议案例', '交易周期', '用户满意度'};											
text_contents4 = {'交易公告发布', '竞价方式', '交易资格审核', '交易合同管理', '支付方式', '交易完成确认', '交易纠纷处理'};											
text_contents5 = {'交易流程效率', '合同履行率', '交易安全性', '法律合规性', '市场适应性', '用户参与度', '交易满意度'};											
...
features2 = [
    struct('type', 'popupmenu', 'items', {formatItems('自动|手动')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('即时|定期')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('电子|纸质|混合')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('在线|线下|混合')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('有|无')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('严格|一般|宽松')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('强|中|弱')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
];											
											
features3 = [
    struct('type', 'edit', 'fontSize', 12, 'width', 60, 'unit', '个', 'unitFontSize', 14, 'defaultValue', '', 'items', {{}}, 'range', []),...
    struct('type', 'edit', 'fontSize', 12, 'width', 60, 'unit', '万元', 'unitFontSize', 14, 'defaultValue', '', 'items', {{}}, 'range', []),...
    ...
    struct('type', 'edit', 'fontSize', 12, 'width', 60, 'unit', '个', 'unitFontSize', 14, 'defaultValue', '', 'items', {{}}, 'range', []),...
    struct('type', 'edit', 'fontSize', 12, 'width', 60, 'unit', '天', 'unitFontSize', 14, 'defaultValue', '', 'items', {{}}, 'range', []),...
    struct('type', 'edit', 'fontSize', 12, 'width', 60, 'unit', '%', 'unitFontSize', 14, 'defaultValue', '', 'items', {{}}, 'range', []),...
];											
											
features4 = [
    struct('type', 'popupmenu', 'items', {formatItems('是|否')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('公开竞价|封闭竞价')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('严格|宽松')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('标准合同|定制合同')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('现金|银行转账|在线支付')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('是|否')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('在线调解|法律仲裁')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
];											
...
handles = createUIComponents(handles, handles.uipanel5, features4, 39, 160, [255, 255, 255], [0, 0, 0]);
...
surf(x, y, z)
xlabel(handles.axes2,'交易数量');
ylabel(handles.axes2,'交易金额');
zlabel(handles.axes2,'交易效率');
title('土地使用权交易效果分析三维图'); 
drawnow; pause(1); 
...
set(gca,'XTickLabel', {'交易流程效率', '合同履行率', '交易安全性', '法律合规性', '市场适应性', '用户参与度', '交易满意度'});
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
h = msgbox('土地使用权交易管理系统为用户提供了交易类型管理、交易管理设置、交易数据管理、交易流程设置、交易效果评估等功能。', '关于本软件','help');
jMsgBox = h.JavaFrame.getFigurePanelContainer.getComponent(0).getTopLevelAncestor;
jMsgBox.setAlwaysOnTop(true);
figure(handles.figure1);
drawnow; pause(1); 
...
% UIWAIT makes Land_Use_Rt_Transact_Mgmt_Sys wait for user response (see UIRESUME)
...
function setImageToAxesWithAlpha(handles, imageName, axesTag, backgroundColor)
    % 构建图片路径 , 'Land_Use_Rt_Transact_Mgmt_Sys'  Administrati
    if isdeployed
