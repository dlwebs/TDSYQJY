function varargout = Sustainable_Development(varargin)
% SUSTAINABLE_DEVELOPMENT MATLAB code for Sustainable_Development.fig
%      SUSTAINABLE_DEVELOPMENT, by itself, creates a new SUSTAINABLE_DEVELOPMENT or raises the existing
%      singleton*.
%
%      H = SUSTAINABLE_DEVELOPMENT returns the handle to a new SUSTAINABLE_DEVELOPMENT or the handle to
%      the existing singleton*.
%
%      SUSTAINABLE_DEVELOPMENT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SUSTAINABLE_DEVELOPMENT.M with the given input arguments.
%
%      SUSTAINABLE_DEVELOPMENT('Property','Value',...) creates a new SUSTAINABLE_DEVELOPMENT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Sustainable_Development_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Sustainable_Development_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Sustainable_Development

% Last Modified by GUIDE v2.5 15-May-2024 16:13:58

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Sustainable_Development_OpeningFcn, ...
                   'gui_OutputFcn',  @Sustainable_Development_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Sustainable_Development is made visible.
function Sustainable_Development_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Sustainable_Development (see VARARGIN)

% Choose default command line output for Sustainable_Development
...
function varargout = Sustainable_Development_OutputFcn(hObject, eventdata, handles) 
...
global ELEMENT_TOP;
ELEMENT_TOP = 300;
set(handles.text26, 'String', '  土地资源可持续发展利用分析软件');
handles.uipanel2.Title = '土地资源利用范围管理';
handles.uipanel3.Title = '土地资源利用设置';
handles.uipanel4.Title = '土地资源保护措施管理';
handles.uipanel5.Title = '土地资源参数设置';
handles.uipanel6.Title = '土地资源利用效果评估';
set(handles.pushbutton8, 'String', '确认选择');
set(handles.pushbutton9, 'String', '保存设置');
set(handles.pushbutton10, 'String', '保存设置');
set(handles.pushbutton11, 'String', '保存设置');
set(handles.pushbutton12, 'String', '利用效果');
set(handles.pushbutton5, 'String', '导入土地资源利用流程图');
set(handles.pushbutton6, 'String', '绘制土地资源利用效果分析三维图');
set(handles.pushbutton7, 'String', '绘制土地资源利用效果评估分析柱状图');
unit_content = {'', '', '', '', '%'};
text_contents1 = {'农业用地', '工业用地', '商业用地', '居住用地', '公共设施用地', '生态保护用地', '交通运输用地'};
text_contents2 = {'土地规划', '土地开发', '土地利用强度', '土地利用模式', '土地资源管理', '土地利用评估', '土地资源保护'};
text_contents3 = {'生态恢复措施', '环境治理措施', '资源节约措施', '土地修复措施', '资源利用技术', '资源管理制度', '资源保护资金'};
text_contents4 = {'利用总面积', '保护总面积', '经济效益总额', '环境改善总评估', '社会效益总评估', '平均资源利用率', '最高利用强度'};
text_contents5 = {'资源利用率', '生态保护效果', '环境改善效果', '经济效益', '社会效益', '土地利用强度', '土地可持续发展'};
features1 = [
...
features2 = [
    struct('type', 'popupmenu', 'items', {formatItems('科学规划|优化配置|合理利用')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('可持续开发|资源节约|环境保护')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('低强度|中强度|高强度')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('集约利用|粗放利用|混合利用')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('统一管理|分级管理|自主管理')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('环境影响|经济效益|社会效益')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('生态保护|资源修复|环境改善')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
];
features3 = [
    struct('type', 'popupmenu', 'items', {formatItems('植树造林|土壤改良|水资源保护')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('污染治理|垃圾处理|废水处理')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('节水灌溉|节能减排|再生资源利用')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('土壤修复|植被恢复|生态修复')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('先进技术|传统技术|混合技术')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('法律法规|政策指导|技术规范')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
...
handles = createDynamicText(handles, handles.uipanel6, text_contents5, 14, 10, 39);

%  功能点值创建（值）的调用方法
handles = createUIComponents(handles, handles.uipanel2, features1, 39, 200);
handles = createUIComponents(handles, handles.uipanel3, features2, 39, 160);
...
handles = createUIComponents(handles, handles.uipanel6, features5, 39, 185);
...
axes(handles.axes2);
% 创建数据
X = linspace(0.1, 1, 30);  % 资源利用率
Y = linspace(1000, 10000, 30);  % 经济效益
[Z, W] = meshgrid(X, Y);
V = Z .* sin(W / 1000) + W / 10000;

% 创建三维图
waterfall(handles.axes2, Z, W, V);

% 设置坐标轴标签和标题
xlabel(handles.axes2, '资源利用率');
ylabel(handles.axes2, '经济效益');
zlabel(handles.axes2, '环境改善效果');
title(handles.axes2, '土地资源利用效果分析三维图');
...
set(gca,'XTickLabel', {'资源利用率', '生态保护效果', '环境改善效果', '经济效益', '社会效益', '土地利用强度', '土地可持续发展'});
set(gca,'XTickLabelRotation',25);
ylim([0,max(max(Y)+max(Y)*0.3)]);
title('土地资源利用效果评估分析柱状图'); 
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
h = msgbox('土地资源可持续发展利用分析软件为用户提供了土地资源利用范围管理、土地资源利用设置、土地资源保护措施管理、土地资源参数设置、土地资源利用效果评估等功能。', '关于本软件','help');
jMsgBox = h.JavaFrame.getFigurePanelContainer.getComponent(0).getTopLevelAncestor;
jMsgBox.setAlwaysOnTop(true);
figure(handles.figure1);
drawnow; pause(1); 
...
% UIWAIT makes Sustainable_Development wait for user response (see UIRESUME)
